...
module.exports = {
  ...
  create(context) {
    const importLevelFromOption = context.options[0]
      ? context.options[0].moduleImportLevels
      : [];
    const {glob, modulesPath, alias, moduleLayers, moduleImportLevels} = {
      ...DEFAULT_OPTIONS,
      ...context.options[0],
      moduleImportLevels: {...importLevelFromOption, ...DEFAULT_OPTIONS},
    };

    if (!alias) {
      throw Error(
        'You have to set an `alias` option in the `module-engagement` rule!',
      );
    }

    if (!modulesPath) {
      throw Error(
        'You have to set a proper `modulesPath` option in the `module-engagement` rule!',
      );
    }
